...
const ShoppingCart = () => {
  ...
  const handleCheckoutRedirect = () => {
    ...
    // 3. ПРЕОБРАЗУЕМ "ua" в "uk" для Shopify и добавляем в URL
    const shopifyLocale = language === "uk" ? "uk" : "en";
    
    const checkoutUrl = `https://${storeDomain}/cart/${cartItems}?checkout[email]=${encodeURIComponent(userEmail)}${addressParams}&locale=${shopifyLocale}`;
  ...
  return (
    <>
      ...
      <aside className={`${styles.sidebar} ${open ? styles.show : ""} flex flex-col`}>
        ...
        {lines.length > 0 && <h2 className={styles.heading}>{t('common.cart.title')}</h2>}

        <div className="flex-1 overflow-y-auto">
          {lines.length === 0 ? (
            <div className={styles.emptyWrapper}>
              <h2 className={styles.heading}>{t('common.cart.empty')}</h2>
              <div className={styles.buttonWrapper}>
                <DefaultButton href="/products" label={t('common.cart.continue')} />
                <h2 className={styles.heading + " mt-8"}>{t('account.have_account')}</h2>
                <Link className={styles.link} href="account/login">
                  <span className={styles.text}>{t('account.go_to_login')}</span>
                </Link>
              </div>
            </div>
          ) : (
                      ...
                      <div className={styles.infoWrapper}>
                        ...
                        <div className={styles.manipulateBtnsWrapper}>
                          <div className={styles.quantityWrapper}>
                            <button className={styles.quantityBtn} onClick={() => handleUpdateItem(line.id, (quantity ?? 1) - 1)}>-</button>
          ...
          <div className="border-t border-gray-200 bg-white relative z-[99999] flex flex-col">
              ...
              <div className="border-b border-black bg-gray-50 relative z-[160]">
                ...
                >
                  <span className="text-[10px] font-bold uppercase tracking-widest text-black text-left">
                    {activeAddress 
                      ? `${t('account.addresses')}: ${activeAddress.city}, ${activeAddress.address1}` 
                      : t('account.addresses')}
                  </span>
            ...
            <div className={styles.checkoutWrapper}>
              <div className={styles.wrapperPrice}>
                <p className={styles.subtotal}>{t("common.cart.subtotal")}</p>
                  ...
                  <li key={line.id} className={styles.cartItem}>
                    <div className="relative w-[50px] h-[50px] flex-shrink-0">
                      <Image 
                        ...
                        src={imageSrc} 
                        alt={merchandise.title || "product image"} 
                        fill
...
const ShoppingCart = () => {
  ...
  const handleUpdateItem = (lineId: string, quantity: number) => {
    if (quantity < 1) return;
    const currentLine = lines.find(line => line.id === lineId);
    updateItem(lineId, quantity, currentLine?.attributes ?? []);
  };
          ...
          {lines.length === 0 ? (
            ...
            <ul className={styles.itemsWrapper}>
              {lines.map(line => {
                const { merchandise, quantity } = line;
                const imageSrc = merchandise.image?.url || "/images/placeholder.png";
                const lineTotal = Number(merchandise.priceV2?.amount || 0) * (quantity ?? 0);

                return (
                  <li key={line.id} className={styles.cartItem}>
                    <div className="relative w-[50px] h-[50px] flex-shrink-0 bg-gray-50 overflow-hidden">
                      <Image 
                        className={styles.image} 
                        src={imageSrc} 
                        alt={merchandise.title || "product"} 
                        fill
                        sizes="50px"
                        style={{ objectFit: "cover" }}
                      />
                    ...
                    <div className={styles.itemInfo}>
                      ...
                      <p className={styles.price}>{lineTotal.toLocaleString()} {merchandise.priceV2?.currencyCode}</p>
                    </div>
                  </li>
                );
              })}
